...
async function withRunningCopy(run) {
  ...
  const root = path.join(await mkdtemp(path.join(tmpdir(), "akeso-e2e-")), "app");
...
test("the whole loop: a broken app is graded F, repaired, and proven A", async () => {
...
test("the generated handler rejects a forged event", async () => {
  await withRunningCopy(async (root) => {
    const detection = await detect(root);
...
test("the generated code says out loud that it is generated", async () => {
  await withRunningCopy(async (root) => {
    const plan = buildFixPlan({ detection: await detect(root) });
    const applied = await applyFixPlan(root, plan, { stamp: "e2e-marker" });
    for (const file of applied.written) {
      const contents = await readFile(path.join(root, file.path), "utf8");
      assert.match(contents, /DO NOT EDIT/, `${file.path} must be marked as generated`);
    }
  });
});
