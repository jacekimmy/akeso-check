...
await writeFile(process.env.AKESO_FIXTURE_DB, JSON.stringify({
  tables: {
    profiles: ACCOUNTS.map((id) => ({ id, is_pro: false, admin_block: false, abuse_block: false, complimentary_access: false, billing_override: false })),
...
http.createServer(async (req, res) => {
  ...
  res.writeHead(404);
  res.end();
}).listen(PORT, () => console.log(`repairable-app listening on ${PORT}`));
